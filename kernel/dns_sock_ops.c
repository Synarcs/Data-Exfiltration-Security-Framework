...
// return infor for succe sock layer reidrect to user sapace same ifindex listener of userspace l7 proxies 
static 
__always_inline bool __dns_tcp_sock_dpi(struct __sk_buff *skb) {
    // perform dpi for envoy integrated uapi sock listener for processing malicious DNS transfer over tcp 
    return true;
}
        ...
        case AF_INET:  // IPv4 packets
            ...
            if ((void *)(iph + 1) > data_end) return SK_DROP; 
                ...
                if (iph->protocol == IPPROTO_TCP) {
                    ...
                    if ((void *)(tcp + 1) > data_end) return SK_DROP;
