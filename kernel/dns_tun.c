...
int classify(struct __sk_buff *skb){    
    ...
    #ifdef DEBUG 
        if (DEBUG) {
            __u32 ifindex = skb->ifindex; // for post egress packet processing 
            bpf_printk("the link index is tun/tap and used for encap should not be used for transfer \
                    kernel dropping the packet %d", ifindex);
        }
    #endif
    return TC_DROP;
}
