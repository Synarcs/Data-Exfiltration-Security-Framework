...
static
__always_inline __u8 __parse_vxlan_flag__hdr(const void *transport_data, struct vxlanhdr *vxlan_hdr, const void *data_end) {
...
static 
__always_inline __u32 __parse_vxlan_vni_hdr(const void *transport_payload, struct vxlanhdr *vxlan_hdr, const void *data_end) {
    __u32 __maybe_unused vni = bpf_ntohs(vxlan_hdr->vx_vni) >> 8;  
    __u32 __maybe_unused resrvered = bpf_ntohs(vxlan_hdr->vx_vni) & 0xff;
    #if DEBUG
