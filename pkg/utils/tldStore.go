...
func VerifyTopDomainsData() (string, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			...
			Log("Runtime permission error please check the file stats and permission", err)
		}
		return "", err
	}
...
func ReadTldDomainsData() (*TopDomains, error) {
	...
	if DEBUG {
		...
		Log("Total domains found", length)
	}

	Log("File exists and Read via Parallel I/O for file stats", fileInfo.Name(), fileInfo.Size()/(1<<10)*3)
...
func (t *TopDomains) UpdateDomainDomainTLDCache(domain string) {
	if _, fd := t.TopDomains.Load(domain); fd {
		Log("the Required domain already present as safe TLD in Node Agent Cache ", domain)
		return
	}

	if len(domain) == 0 || strings.Count(domain, ".") != 1 || len(strings.Split(domain, ".")) != 2 {
		Log("cannot unblock an malformed SLD in the cache")
		return
	}
