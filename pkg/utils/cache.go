...
/*
	Core caching layer of the eBPF endpoint agent in userspace
...
func InitCache(conf *CacheConfig) error {
	...
	NODE_AGENT_BLACKLISTED_DOMAINS = expirable.NewLRU[string, *expirable.LRU[string, bool]](
		...
		nil,
		time.Second*time.Duration(conf.EgressMaliciousCacheTTL),
	)
	// init the init ingress cache
	NODE_AGENT_INGRESS_BACKLISTED_DOMAINS = expirable.NewLRU[string, bool](MAX_NODE_AGENT_CACHE_SIZE, nil, time.Second*time.Duration(conf.IngressMaliciousCacheTTL))

	// this is cache without ttl since the implant malware cannot brute force this to bring agent down
...
func UpdateDomainNestedEgressCache(tld, fqdn string, isBlackListEgress bool) {
	if isBlackListEgress {
		...
		if !fd {
			newDomainCache := expirable.NewLRU[string, bool](INFERENED_DOMAIN_CACHE_SIZE_PER_TLD, nil, time.Second*50) // TODO: fix this to be configured but nested cache for egress must be half the life of parent
